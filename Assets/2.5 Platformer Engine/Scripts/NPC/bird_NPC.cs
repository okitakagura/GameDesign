...
public class bird_NPC : MonoBehaviour
{

    //定义NPC对话数据
    private string[] mData = { "Hello, I'm the housekeeper here —— Bird_Army.","My master and his wife are all painters.", "They are sensitive to colors.","They take good care of me and I miss them so much.","Can you find my master and complete his wishes？" };
    //当前对话索引
    private int index = 0;
    //用于显示对话的GUI Text
    public Text mText;
        ...
        if (Vector3.Distance(transform.position, player.transform.position) < 2)
        {
            ...
            if (Vector3.Distance(transform.position, mousePositionInWorld) < 1)
            {
                ...
                if (Input.GetKeyDown(KeyCode.Alpha0) || Input.GetMouseButtonDown(0))
                {
                    ...
                    {
                        mText.text = "Bird_Army:" + mData[index];
                        index = index + 1;
                    }
                    else
                    {
